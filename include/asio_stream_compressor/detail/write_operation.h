...
{
public:
    using self = async_write_some_operation<Stream, Core, Handler, ConstBufferSequence>;

    async_write_some_operation(Stream& stream, Core& core,
    ...
    void operator()(error_code ec, std::size_t /*bytes_transferred*/ = std::size_t(0))
    {
        do
        {
            ...
            case state::send_data:
            {
                ...
                asio::async_write(stream_.next_layer(),
                                  core_.write_buf_.data(),
                                  std::move(*this));
                return;
            }
            ...
            case state::pass_data_to_handler:
            {
                ...
                core_.pending_write_.expires_at(neg_infin());
                core_.stats_.tx_bytes_total.fetch_add(buffers_.size(), std::memory_order_relaxed);
                core_.stats_.tx_bytes_compressed.fetch_add(core_.write_buf_.size(), std::memory_order_relaxed);
    ...
    void encode_data()
    {
        for (auto& in : buffers_)
        {
            ...
            while (in_buf.pos != in_buf.size)
            {
                ...
                size_t compress_result = ZSTD_compressStream2(core_.cctx_.get(), &out_buf, &in_buf, ZSTD_EndDirective::ZSTD_e_continue);
        ...
        {
            ZSTD_outBuffer out_buf = get_free_buffer();
            compress_result = ZSTD_compressStream2(core_.cctx_.get(), &out_buf, &in_buf, ZSTD_EndDirective::ZSTD_e_flush);
            core_.write_buf_.commit(out_buf.pos);
            if (check_set_error(compress_result))
                return;
