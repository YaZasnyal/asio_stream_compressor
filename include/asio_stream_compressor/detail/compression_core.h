...
public:
    ...
    void reset()
    {
        ZSTD_CCtx_reset(cctx_.get(), ZSTD_ResetDirective::ZSTD_reset_session_and_parameters);
        ZSTD_DCtx_reset(dctx_.get(), ZSTD_ResetDirective::ZSTD_reset_session_and_parameters);
    ...
    timer pending_read_; ///< @brief Timer used for storing queued read operations
    timer pending_write_; ///< @brief Timer used for storing queued write operations
    asio::basic_streambuf<Allocator> input_buf_; ///< @brief buffer for input data from next_layer
    asio::basic_streambuf<Allocator> write_buf_; ///< @brief buffer for output data
