...
{
public:
    using self = async_read_some_operation<Stream, Core, Handler, MutableBufferSequence>;

    async_read_some_operation(Stream& stream, Core& core,
        ...
        do
        {
            ...
            case state::decode_data:
            {
                ...
                if (bytes_transferred != 0)
                {
                    ...
                    core_.stats_.rx_bytes_compressed.fetch_add(bytes_transferred, std::memory_order_relaxed);
                }
            ...
            {
                core_.pending_read_.expires_at(neg_infin());
                core_.stats_.rx_bytes_total.fetch_add(bytes_written_, std::memory_order_relaxed);
        ...
        for (auto out = buffers_.begin(); out != buffers_.end();)
        {
            ...
            do
            {
                ...
                if (core_.input_buf_.size() == 0)
                {
                    ...
                    decompression_result = ZSTD_decompressStream(core_.dctx_.get(), &out_buf, &in_buf);
                    break;
                }
                else
                {
...
public:
    ...
    {
        using read_op = async_read_some_operation<
