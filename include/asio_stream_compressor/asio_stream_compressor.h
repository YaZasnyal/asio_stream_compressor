...
public:
     ...
     */
    template <typename Executor>
    compressor(Executor& ex, int level, const Allocator& alloc = Allocator()) noexcept(false)
        : next_layer_(Stream(ex))
     ...
     * @param arg - object that implements AsyncReadStream and AsyncWriteStream traits
     * @param alloc - custom allocator
     *
     * @throws system_error with zstd exception if level is incorrect
     */
    compressor(Stream&& next_layer, const Allocator& alloc = Allocator()) noexcept(false)
     ...
     * @throws system_error with zstd exception if level is incorrect
     */
    compressor(Stream&& next_layer, int level = ZSTD_defaultCLevel(), const Allocator& alloc = Allocator()) noexcept(false)
        : next_layer_(std::move(next_layer))
        , core_(level, next_layer_.get_executor(), alloc)
    {}
     ...
     * @tparam ReadToken - completion handler with signature @code void (error_code, std::size_t) @endcode
     *
    ...
    template <typename MutableBufferSequence,
              typename ReadToken = typename asio::default_completion_token<executor_type>::type>
    auto async_read_some(const MutableBufferSequence& buffers,
                         ReadToken&& token = typename asio::default_completion_token<executor_type>::type())
    {
        return asio::async_initiate<ReadToken, void (error_code, std::size_t)>(
                    detail::initiate_async_read_some<self, decltype(core_)>(*this, core_), token, buffers);
    }
    ...
    template <typename ConstBufferSequence,
              typename WriteToken = typename asio::default_completion_token<executor_type>::type>
    auto async_write_some(const ConstBufferSequence& buffers,
                          WriteToken&& token = typename asio::default_completion_token<executor_type>::type())
    {
      return asio::async_initiate<WriteToken, void (error_code, std::size_t)>(
                  detail::initiate_async_write_some<self, decltype(core_)>(*this, core_), token, buffers);
    }
     ...
     * undefined behavior. Use reset() to reset internal context and configure context
     * again.
     *
     * Example:
     * @endcode
