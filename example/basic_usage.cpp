...
auto main() -> int
{
    ...
    try
    {
        ...
        sock.next_layer().async_connect(
                    ...
                    asio_stream_compressor::asio::use_future).get();

        // send data
        auto bytes = asio_stream_compressor::asio::async_write(sock, sb.data(),
                                                        asio_stream_compressor::asio::use_future).get();
        ...
        bytes = asio_stream_compressor::asio::async_read(sock, bufs,
                                                  asio_stream_compressor::asio::use_future).get();
        sb.commit(bytes);
    ...
    std::cout << "stats: \n"
              ...
              << "\n\trx_bytes_compressed: " << stat.rx_bytes_compressed.load()<< std::endl;
